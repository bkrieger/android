...
import us.happ.R;
import us.happ.fragment.BoardFragment;
import us.happ.fragment.ContactsFragment;
import us.happ.fragment.FeedbackFragment;
import us.happ.fragment.HappFragment;
import us.happ.gcm.GcmIntentService;
import us.happ.service.APIService;
import us.happ.service.ServiceHelper;
import us.happ.service.ServiceReceiver;
import us.happ.utils.ContactsManager;
import us.happ.utils.Storage;
import us.happ.view.Drawer;
...
public class MainActivity extends ActionBarActivity implements ServiceReceiver.Receiver {
	...
	public void startFetchService(){
		...
		// Retry services that failed previously
		if (Storage.getRetryUpdateFriends(this)){
			startUpdateFriendsService();
		}
	}
	...
	public void startUpdateFriendsService(){
		Bundle extras = new Bundle();
		String[] numbers = mContactsManager.getAllFriends();
		extras.putStringArray("n", numbers);
		extras.putString("number", mPhoneNumber+"");
		extras.putParcelable(ServiceReceiver.NAME, (Parcelable) mReceiver);
        ServiceHelper mServiceHelper = ServiceHelper.getInstance();
        postFriendsId = mServiceHelper.startService(this, ServiceHelper.POST_FRIENDS, extras);
	}
	...
	//					Toast.makeText(this, getResources().getString(R.string.toast_post_success), Toast.LENGTH_SHORT).show();
			...
			postFeedbackId = -1;
		} else if (taskId == postFriendsId){
			Storage.setRetryServiceUpdateFriends(this, !isRequestSuccessful(results));
			postFriendsId = -1;
		}
	...
	private boolean isRequestSuccessful(String results){
		try{
			if (results != null){
				JSONObject jResults = new JSONObject(results);
				if (jResults.getInt("status") == 200){
    ...
    private boolean checkFirstTimeUser(){
    	if (!Storage.allowContactsAccess(this)){
